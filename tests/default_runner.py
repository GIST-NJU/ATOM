...
def runner1(dataloader_class, model_class, args):
    val_dataset = dataloader_class(args.data, phase=args.phase, followup=args.followup, inp_name=args.inp_name)
...
def create_loader_and_model(dataloader_class, model_class, args, part: (int, int) = None):
    if args.model_name == 'mldecoder':
        model = model_class(args)
        if part is None:
            val_dataset = dataloader_class(
                args.data, phase=args.phase,
                followup=args.followup, inp_name=args.inp_name,
            ...
            val_dataset = dataloader_class(
                args.data, phase=args.phase,
                followup=args.followup, part=part, inp_name=args.inp_name,
                transform=transforms.Compose([
                    transforms.Resize((args.image_size, args.image_size)),
                    transforms.ToTensor(),
                ])
...
def runner2(dataloader_class, model_class, args):
    if args.__contains__("part"):
        ...
        for i in range(args.part):
            ...
            val_loader, model = create_loader_and_model(dataloader_class, model_class, args, (i, args.part))
            if args.model_name == 'mldecoder':
            ...
            maps.append(map_i)
            result_i.to_excel(os.path.join('tmp', 'result_{dataname}_{model_name}_p{part}.xlsx'.format(
                dataname=args.data_name, model_name=args.model_name, part=i
            )))
