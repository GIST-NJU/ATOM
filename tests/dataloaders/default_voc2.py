...
import pickle
from . import default_util as util
...
def read_object_labels_csv(file, header=True, followup=False, src_path=None, dst_path=None):
    images = []
    labels_list = []
    num_categories = 0
    print('[dataset] read', file)
    ...
    if followup:
        ...
        images = img_followup_list(images, src_path, dst_path)
        images = [".".join(img.split(".")[:-1]) for img in images]
        labels_list = [val for val in labels_list for i in range(7)]
    return list(zip(images, labels_list))
...
class Voc2007Classification2(data.Dataset):
    def __init__(self, root, phase, followup=False, transform=None, target_transform=None, inp_name=None, adj=None):
        self.root = root
        ...
        self.images = read_object_labels_csv(file_csv, followup=self.followup, src_path=self.path_images, dst_path=self.followup_path)
    ...
    def __getitem__(self, index):
        ...
        # img = None
        if self.followup:
            img = Image.open(os.path.join(self.followup_path, path + '.png')).convert('RGB')
        else:
