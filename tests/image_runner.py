...
def runner1(dataloader_class, model_class, args):
    val_dataset = dataloader_class(
        args.data, phase=args.phase, image_num=args.image_num, followup=args.followup, inp_name=args.inp_name
    )
...
def create_model(dataloader_class, model_class, args, part: (int, int) = None):
    if args.model_name == 'mldecoder':
        ...
        if part is None:
            ...
            val_dataset = dataloader_class(
                args.data, part=part, phase=args.phase, image_num=args.image_num, followup=args.followup, inp_name=args.inp_name,
                transform=transforms.Compose([
                    transforms.Resize((args.image_size, args.image_size)),
                    transforms.ToTensor(),
                ])
...
def runner(args):
    ...
    for i in range(args.start_no, args.repeat + 1):
        args.phase = "{}{}_No{}".format(
            args.data_name, args.way_num, i
        )
        if args.model_name in ['msrn', 'mlgcn', 'dsdl', 'mcar']:
            result, _ = runner1(args.dataloader, model, args)
        elif args.model_name in ['mldecoder', 'asl']:
            result, _ = runner2(args.dataloader, model, args)
        else:
            raise NotImplementedError("model {} is not implemented".format(args.model_name))
        result_file = os.path.join(
            args.res_path,
            r"result_{}{}_No{}".format(
                args.data_name, args.way_num, i
            ),
            r'result_{dataname}_{way_num}way_{model_name}.xlsx'.format(
                dataname=args.data_name, way_num=args.way_num, model_name=args.model_name
            )
        )
