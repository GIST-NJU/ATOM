...
class MCARMobilenetv2(nn.Module):
    ...
    def forward(self, inputs):
        ...
        linputs = torch.zeros([b, self.topN, 3, h, w])
        if self.vis == True:
...
class MCARResnet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # local stream
        linputs = linputs.view(b * self.topN, 3, h, w)         
        la = self.features(linputs.detach().cuda())
        lf = feats_pooling(la, method=self.ps, sh=int(h/32), sw=int(w/32))
        lf = self.convclass(lf)
        ls = torch.sigmoid(lf)
        ls = F.max_pool2d(ls.reshape(b, self.topN, self.num_classes, 1).permute(0,3,1,2), (self.topN, 1))
...
def MCAR(args):
    ...
    if args.bm == 'resnet101':
        model = mcar_resnet101(num_classes=args.num_classes, ps=args.ps, topN=args.topN, threshold=args.threshold,
                               pretrained=True)
        args.epoch_step = [30, 50]
        args.lrp = 0.1
        args.lr = 0.01
    elif args.bm == 'resnet50':
        model = mcar_resnet50(num_classes=args.num_classes, ps=args.ps, topN=args.topN, threshold=args.threshold,
                              pretrained=True)
        args.epoch_step = [30, 50]
        args.lrp = 0.1
        args.lr = 0.01
    elif args.bm == 'mobilenetv2':
        model = mcar_mobilenetv2(num_classes=args.num_classes, ps=args.ps, topN=args.topN, threshold=args.threshold,
                                 pretrained=True)
        ...
        raise Exception('no such backbone model: {}'.format(args.bm))
