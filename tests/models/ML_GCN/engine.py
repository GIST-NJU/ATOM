...
class Engine(object):
    ...
    def predict(self, model, criterion, val_dataset, optimizer=None):
        self.init_learning(model, criterion)
        ...
        if self.state['evaluate']:
            self.validate_google(val_loader, model, criterion)
            return

    def validate_google(self, data_loader, model, criterion):
...
class MultiLabelMAPEngine(Engine):
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        # OP_k, OR_k, OF1_k, CP_k, CR_k, CF1_k =  0,0,0,0,0,0
        if display:
            if training:
                print('Epoch: [{0}]\t'
                      'Loss {loss:.4f}\t'
    ...
    def on_end_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        self.state['names'] += self.state['out']
        if display and self.state['print_freq'] != 0 and self.state['iteration'] % self.state['print_freq'] == 0:
            loss = self.state['meter_loss'].value()[0]
            batch_time = self.state['batch_time'].value()[0]
            data_time = self.state['data_time'].value()[0]
